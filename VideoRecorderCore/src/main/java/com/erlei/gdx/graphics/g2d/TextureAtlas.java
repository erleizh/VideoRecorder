...
public class TextureAtlas implements Disposable {
	...
	public static class TextureAtlasData {
		public static class Page {
			...
			public Page (FileHandle handle, float width, float height, boolean useMipMaps, Pixmap.Format format, TextureFilter minFilter,
		...
		public TextureAtlasData (FileHandle packFile, FileHandle imagesDir, boolean flip) {
			...
			Collections.sort(regions,indexComparator);
		}
	...
	private void load (TextureAtlasData data) {
		HashMap<TextureAtlasData.Page, Texture> pageToTexture = new HashMap<TextureAtlasData.Page, Texture>();
	...
	public List<AtlasRegion> findRegions (String name) {
		List<AtlasRegion> matched = new ArrayList<>();
		for (int i = 0, n = regions.size(); i < n; i++) {
			AtlasRegion region = regions.get(i);
	...
	public List<Sprite> createSprites () {
		List<Sprite> sprites = new ArrayList(regions.size());
		for (int i = 0, n = regions.size(); i < n; i++)
	...
	public List<Sprite> createSprites (String name) {
		List<Sprite> matched = new ArrayList();
		for (int i = 0, n = regions.size(); i < n; i++) {
			AtlasRegion region = regions.get(i);
