...
import com.erlei.gdx.utils.MipMapGenerator;
...
public abstract class GLTexture implements Disposable {
    ...
    public GLTexture(int glTarget) {
        this(glTarget, GLUtil.glGenTexture());
    }
    ...
    /**
     * Binds the texture to the given texture unit. Sets the currently active texture unit via {@link GLES20#glActiveTexture(int)}.
     *
    ...
    protected void delete() {
        if (glHandle != 0) {
            GLUtil.glDeleteTexture(glHandle);
            glHandle = 0;
        }
    }
