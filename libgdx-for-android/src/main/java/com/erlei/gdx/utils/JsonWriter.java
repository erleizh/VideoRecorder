...
// TODO: 2018/9/24 java 中的StringBuilder 和Android中的StringBuilder有差异，替换了，但是没有经过测试
//
//
...
//        }
        ...
        public String quoteValue(Object value) {
            ...
            return '"' + replaceAll(buffer, "\"", "\\\"").toString() + '"';
        }
        ...
        public StringBuilder replaceAll(StringBuilder sb, String find, String replace) {
            int findLength = find.length(), replaceLength = replace.length();
            int index = 0;
            while (true) {
                index = sb.indexOf(find, index);
                if (index == -1) break;
                sb.replace(index, index + findLength, replace);
                index += replaceLength;
            }
            return sb;
        }
