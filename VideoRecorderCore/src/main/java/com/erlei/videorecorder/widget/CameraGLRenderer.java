...
public class CameraGLRenderer implements SurfaceTexture.OnFrameAvailableListener {
    ...
    private final CameraGLView mView;
    ...
    private boolean mHaveSurface, mIsStarted, mHaveFBO, mUpdateST;
    ...
    public CameraGLRenderer(CameraGLView view) {
        this(view, null);
    }

    public CameraGLRenderer(CameraGLView view, CoordinateTransform transform) {
        mView = view;
    ...
    public void onSurfaceChanged(int width, int height) {
        LogUtil.logd("onSurfaceChanged(" + width + "x" + height + ")");
        mHaveSurface = true;
        updateState();
        setPreviewSize(mView.getSurfaceSize());
    }

    public synchronized void updateState() {
        LogUtil.logd("updateState");
        boolean willStart = mHaveSurface && mView.isEnabled() && mView.getVisibility() == View.VISIBLE;
        if (willStart != mIsStarted) {
            if (willStart) doStart();
            else doStop();
        } else {
            LogUtil.logd("keeping State unchanged");
        }
        LogUtil.logd("updateState end");
    }
    ...
    protected void doStop() {
        ...
        CameraGLView.CameraTextureCallback callback = mView.getCameraTextureCallBack();
        if (callback != null) {
    ...
    protected synchronized void doStart() {
        LogUtil.logd("doStart");
        initSurfaceTexture();
        mIsStarted = mView.openCamera(mTexture);
        if (mIsStarted) {
    ...
    public void updateCoordinateTransform() {
        ...
        if (mTextureTransform == null) {
            mTextureTransform = new DefaultCoordinateTransform(mView);
        }
    ...
    private synchronized void setPreviewSize(Size size) {
        ...
        CameraGLView.CameraTextureCallback callback = mView.getCameraTextureCallBack();
        if (callback != null) {
            callback.onCameraViewStarted(mSurfaceSize);
        }
    }
    ...
    public void onDrawFrame() {
        if (!mHaveFBO || mTexture == null || !mIsStarted) return;
        synchronized (this) {
            if (mUpdateST) {
