...
public class FacedCubemapData implements CubemapData {
	 ...
	 * @param side The {@link Cubemap.CubemapSide}
	...
	public int getWidth () {
		int tmp, width = 0;
		if (data[Cubemap.CubemapSide.PositiveZ.index] != null && (tmp = data[CubemapSide.PositiveZ.index].getWidth()) > width) width = tmp;
	...
	public void consumeCubemapData () {
		for (int i = 0; i < data.length; i++) {
			if (data[i].getType() == TextureData.TextureDataType.Custom) {
				data[i].consumeCustomData(GLES20.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i);
			} else {
				...
				if (GLUtil.GL_VERSION >=3){
					...
					GLES30.glTexImage2D(GLES20.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, pixmap.getGLInternalFormat(), pixmap.getWidth(),
							pixmap.getHeight(), 0, pixmap.getGLFormat(), pixmap.getGLType(), pixmap.getPixels());
				}else {
					GLES20.glPixelStorei(GLES20.GL_UNPACK_ALIGNMENT, 1);
					GLES20.glTexImage2D(GLES20.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, pixmap.getGLInternalFormat(), pixmap.getWidth(),
							pixmap.getHeight(), 0, pixmap.getGLFormat(), pixmap.getGLType(), pixmap.getPixels());
				}
