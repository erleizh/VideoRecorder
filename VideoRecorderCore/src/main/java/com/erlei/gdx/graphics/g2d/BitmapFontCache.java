...
public class BitmapFontCache {
	...
	private final ArrayList<GlyphLayout> layouts = new ArrayList();
	private final ArrayList<GlyphLayout> pooledLayouts = new ArrayList();
	...
	private IntArrayList[] pageGlyphIndices;
	...
	public BitmapFontCache(BitmapFont font, boolean integer) {
		...
		if (pageCount > 1) {
			...
			pageGlyphIndices = new IntArrayList[pageCount];
			for (int i = 0, n = pageGlyphIndices.length; i < n; i++)
				pageGlyphIndices[i] = new IntArrayList();
		}
	...
	public void tint (Color tint) {
		...
		for (int i = 0, n = layouts.size; i < n; i++) {
			...
			for (int ii = 0, nn = layout.runs.size; ii < nn; ii++) {
				...
				ArrayList<Glyph> glyphs = run.glyphs;
	...
	public void setColors (float color, int start, int end) {
		...
		for (int i = 0; i < pageCount; i++) {
			float[] vertices = pageVertices[i];
			IntArrayList glyphIndices = pageGlyphIndices[i];
	...
	public void draw (Batch spriteBatch) {
		ArrayList<TextureRegion> regions = font.getRegions();
	...
	private void requireGlyphs (GlyphLayout layout) {
		if (pageVertices.length == 1) {
			...
			for (int i = 0, n = layout.runs.size; i < n; i++) {
				ArrayList<Glyph> glyphs = layout.runs.get(i).glyphs;
	...
	private void addToCache (GlyphLayout layout, float x, float y) {
		...
		if (pageVertices.length < pageCount) {
			...
			IntArrayList[] newPageGlyphIndices = new IntArrayList[pageCount];
			...
			for (int i = pageGlyphIndicesLength; i < pageCount; i++)
				newPageGlyphIndices[i] = new IntArrayList();
		...
		for (int i = 0, n = layout.runs.size; i < n; i++) {
			...
			ArrayList<Glyph> glyphs = run.glyphs;
			FloatArrayList xAdvances = run.xAdvances;
	...
	public ArrayList<GlyphLayout> getLayouts () {
		return layouts;
	}
}
